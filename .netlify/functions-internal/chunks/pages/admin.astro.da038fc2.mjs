      ...
      <div id="dashboardSection" class="hidden">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-4 mb-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-2 mb-3">
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">Week ID</label>
              <input id="weekIdInput" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50" placeholder="YYYY-WW">
            </div>
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">Date début</label>
              <input id="weekStartInput" type="date" class="w-full px-3 py-2 border border-gray-300 rounded-md">
            </div>
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">Date fin</label>
              <input id="weekEndInput" type="date" class="w-full px-3 py-2 border border-gray-300 rounded-md">
            </div>
          </div>
          <div class="space-y-2">
            <textarea id="defiInput" placeholder="Une expression par ligne" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 resize-none"></textarea>
            <div class="flex flex-wrap gap-2">
              ...
              <button onclick="reparerMigrationLegacy()" class="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-50 transition-colors">
                Reparer migration legacy
              </button>
            </div>
          </div>
          <div id="legacyRepairStatus" class="mt-2 text-xs text-gray-500"></div>
          <div id="defiActuel" class="mt-3 p-2 bg-gray-50 border border-gray-200 rounded-md">
            <p class="text-sm text-gray-600">Aucune semaine chargée</p>
          </div>
          <div class="mt-3">
            <p class="text-xs font-medium text-gray-700 mb-2">Archives</p>
            <div id="defiArchives" class="flex flex-wrap gap-2"></div>
          </div>
        </div>
  ...
  <div id="addTexteModal" class="fixed inset-0 bg-black/50 hidden flex items-center justify-center z-50">
    <div class="bg-white border border-gray-200 rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto shadow-lg">
      ...
      <form id="texteForm" class="space-y-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Remarque de l'enseignant (optionnel)</label>
          <textarea id="texteRemarqueInput" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ex: Bonne progression sur l'introduction et les connecteurs."></textarea>
        </div>
        <div class="flex gap-2">
